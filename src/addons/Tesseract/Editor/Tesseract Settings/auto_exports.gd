...
func add_item(output_path:String, mod_path:String) -> void:
	var item_data:Array[String] = [output_path, mod_path]
	...
	item.applied.connect(func(applied_output_path:String, applied_mod_path:String) -> void:
		item_data[0] = applied_output_path
		item_data[1] = applied_mod_path
		save()
	)
...
func _on_ae_add_pressed() -> void:
	add_item(%'AE Output Path'.text, %'AE Mod Path'.text)
